...
from .models import Curso, Estudiante, EstudiantePreguntas, EstudiantePruebas, Prueba, Preguntas, Tema
...
def bienvenida(request):
    calcular()
    data1 = Prueba.objects.all()
    data2 = EstudiantePruebas.objects.all()
    data3 = Preguntas.objects.all()
    data4 = Estudiante.objects.all()
    data5 = Tema.objects.all()
    context = {'data1': data1, 'data2': data2, 'data3': data3, 'data4': data4, 'data5': data5}
    return render(request, 'base/bienvenida.html', context)
...
def relacionarPreguntas(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            relacion.prueba_id = a.prueba_id
            relacion.save()
...
def calcular():
    ...
    for x in es:
        ...
        a.save()
    es = EstudiantePreguntas.objects.values('estudiante_id', 'prueba_id').annotate(summary=Sum('correcto')).annotate(count=Count('id'))
    for x in es:
        if x['summary'] < (x['count']*0.6):
            n = (3 * (x['summary'] / (0.6 * x['count']))) + 1
        else:
            n = (3 * ((x['summary'] - (0.6 * x['count'])) / (x['count'] * 0.4))) + 4
        a, created = EstudiantePruebas.objects.update_or_create(prueba_id=x['prueba_id'], estudiante_id=x['estudiante_id'], defaults={'calificacion':n})  
    es = EstudiantePruebas.objects.values('prueba_id').annotate(average=Avg('calificacion'))
    for x in es:
        a = Prueba.objects.get(id=x['prueba_id'])
        a.calificacionprueba = x['average']
        a.save()